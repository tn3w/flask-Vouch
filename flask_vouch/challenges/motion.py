...
from .scoring import SessionStore, score_token, verify_token
...
def challenge_binding(random_data: str) -> str:
    """Ties a score token to the challenge it was issued for."""
...
@dataclass
class MotionAttestation(ChallengeHandler):
    """Clears a visitor on how they moved. No puzzle, nothing to solve.

    ``interactive`` shows a button to click, which is what produces a real
    approach path; switch it off to watch quietly and submit on its own.
    """
    ...
    def challenge_type(self) -> ChallengeType:
        return ChallengeType.MOTION_ATTESTATION

    def generate_random_data(self, difficulty: int = 0) -> str:
        return secrets.token_hex(32)

    def threshold(self, difficulty: int) -> float:
        harder = max(0, difficulty - self.base_difficulty) * 0.02
        return min(0.85, self.min_score + harder)

    def verify(self, random_data: str, nonce: int | str, difficulty: int) -> bool:
        payload = read_attestation(str(nonce), self.secret, random_data)
        if not payload:
            return False
        return payload.get("score", 0) >= self.threshold(difficulty)

    def jwt_extra(self, random_data: str, nonce: int | str) -> dict:
        payload = read_attestation(str(nonce), self.secret, random_data)
        return {"motion": payload["score"]} if payload else {}
    ...
    def handle_http_poll(self, body: dict, engine) -> dict:
        ...
        result = score_motion(body.get("motion"))
        token = score_token(
            result["score"],
            self.secret,
            ttl=ATTESTATION_TTL,
            verdict=result["verdict"],
            cid=challenge_binding(challenge.random_data),
        )
        return {"type": "result", "token": token, "verdict": result["verdict"]}


def open_challenge(engine, challenge_id: str, expected: ChallengeType):
    """The stored challenge, when it is unspent and of the expected kind."""
    challenge = engine.store.get(challenge_id)
    if not challenge or challenge.spent or challenge.challenge_type != expected:
        return None
    return challenge


def read_attestation(token: str, secret: bytes, random_data: str) -> dict | None:
    """Unseal a score token and check it belongs to this challenge."""
    payload = verify_token(token, secret)
    if not payload or time.time() > payload.get("exp", 0):
        return None
    if not hmac.compare_digest(
        str(payload.get("cid", "")), challenge_binding(random_data)
    ):
        return None
    return payload
